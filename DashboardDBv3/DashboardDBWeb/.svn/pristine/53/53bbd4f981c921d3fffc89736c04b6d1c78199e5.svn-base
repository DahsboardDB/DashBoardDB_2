package co.sistemcobro.dashboarddb.filter;
...
import co.sistemcobro.dashboarddb.bean.ClienteBean;
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (loginBean != null && StringUtils.isNoneBlank(loginBean.getUsuario())
			...
			if (Integer.parseInt(usuarioLogin.getCodigoRespuesta()) == 201) {
				logger.info("Usuario no tiene permisos para la aplicación ..");
...
//	if (StringUtils.isNoneBlank(idcliente) && StringUtils.isNoneBlank(idgestion)) {
//		resp.sendRedirect(contextPath + "../../pages/obligacion/obligacion.xhtml"+"?dcliente=" + idcliente + "?idgestion=" + idgestion);
...
//	if (StringUtils.isNoneBlank(idcliente) && StringUtils.isBlank(idgestion)) {
//		ClienteBean clienteBean = (ClienteBean) (req.getSession().getAttribute("clienteBean"));
//		if (clienteBean != null) {
//			clienteBean.init();
//			Cliente cliente = new Cliente();
...
//	}
	...
	public UsuarioHermes autenticacionUsuario(LoginBean loginBean) {
		...
		try {
			...
			//pruebas
			//url = new URL("http://172.16.15.70:8080/hermes/servicios/WS/login");
			
			//producccion 
			url = new URL("http://172.18.89.3:8080/hermes/servicios/WS/login");
